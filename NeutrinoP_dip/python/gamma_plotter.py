...
def Plot(reference, fits):
    ...
    for fit in fits:
        file = root_table + fit + ".dat" 
        data = genfromtxt(file, skip_header=1)
        x_val = data[:,0]
        y_val = data[:,1]
        ax.loglog(x_val, y_val, color = dipole_colors[fit], ls = dipole_style[fit], lw = 2, 
            label = dipole_display[fit], alpha = 1.0)
    ...
    ax.set_ylim(1.e-30, 1.e-28)
    ax.set_xlim(1.e3, 2.e12)
    savefig("/home/mkroll/work/GammaP_dip/plots/test.pdf", bbox_inches='tight')
    cla()
    clf()
    close()
...
if __name__ == "__main__":
    data_list = []
    pdfsets = ["CT10nlo", "HERAPDF15NLO_EIG", "NNPDF23_nlo_as_0118"]
    dipoles = ["gbw", "maddip", "munier", "soyez"]
